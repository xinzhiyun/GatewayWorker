...
class ActionController extends Controller
{
    ...
    {
        // $this->devices_init('868575025659543');

                    // $url='http://waterdispenser.houseselected.com/waterDispenser/open/addRechargeOrder';
                    // $data['phoneNumber']='18002229021';
                    // $data['rechargeAmount']='10';
                    // dump(post($url,$data));
//
...
//        $this->sendMsg($message);
    ...
    // public function RemoteRecharge()
    // {

    //     if (empty($_POST['key'])) {
    //         exit(json_encode(['returnCode'=>-1,'returnMsg'=>"认证错误"]));
    //     } else{
    //         if(empty($_POST['DeviceID']) || empty($sdata['ReDay']) ) {
    //             exit(json_encode(['returnCode'=>-2,'returnMsg'=>"信息错误"]));
    //         }
    //         $key=md5(md5($_POST['DeviceID']).$sdata['ReDay']);
    //     }

    //     $dcode = $_POST['DeviceID'];
     
    //     if(empty(M('devices')->where('DeviceID='.$dcode)->find())) {
    //         exit(json_encode(['returnCode'=>-3,'returnMsg'=>"设备不存在"]));
    //     }

    //     $devicesStatu = M('devicesStatu')->where('DeviceID='.$dcode)->find();
    //     if(empty($devicesStatu)) {
    //         $sdata['DeviceID'] = $dcode;

    //         M('devicesStatu')->add($sdata);
    //     } else {
    //         M('devicesStatu')->where('id='.$devicesStatu['id'])->save($sdata);
    //     }
    ...
    public function receive()
    {
        ...
        Log::write(json_encode($message),'数据包分发');
...
// Log::write(json_encode($message),'数据包111');
    ...
    public function setdataAction($message)
    {
        if (isset($message['PackNum'])) {
            ...
            if ( $message['PackNum'] == 6 ) {
                ...
                M('devices')->where("device_code=" . $DeviceID)->save(['device_statu'=>3]);

                $status_id = M('devices_statu')->where("DeviceID=" . $DeviceID)->save(['data_statu'=>0,'alivestause'=>1]);
            }

            if ( $message['PackNum'] == 7 ) {

                M('devices')->where("device_code=" . $DeviceID)->save(['device_statu'=>2]);

                $status_id = M('devices_statu')->where("DeviceID=" . $DeviceID)->save(['data_statu'=>0,'alivestause'=>0]);
            }
...
//            }
    ...
    public function get_data($data)
    {
        ...
        if(isset($PackNumArr[$data['data_statu']])){
            $msg['PackNum']  = $PackNumArr[$data['data_statu']];
        }
    ...
    {
        Log::write(json_encode($message),'发送数据包到设备');
...
//            $res[] = M('filters')->where($map)->field('timelife,flowlife')->find();
    ...
    public function devices_init($dcode)
    {
        ...
        Log::write(json_encode($message),'设备初始化');
        $this->sendMsg($message);
    }
     ...
     * @return [type] [description]
     *
     * @Author 李振东 lzdong@foxmail.com 2018-04-12
     */
    public function unbing($dcode)
    {
        $message['DeviceID'] = $dcode;
        $message['PackType'] = "SetData";
        $message['Vison']    = 0;
        ...
        $message['AliveStause']=0;
        $message['Reday']     = 0;

        Log::write(json_encode($message),'设备解绑');
        $this->sendMsg($message);
    }
